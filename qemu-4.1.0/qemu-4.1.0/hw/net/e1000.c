...
#define E1000_FLAG_TSO (1 << E1000_FLAG_TSO_BIT)
    ...
    /* add customized temp reg to save GEM information */
    uint32_t rx_qbar; /* GEM Receive Buffer Queue Base Address */
    uint32_t tx_qbar; /* GEM Transmit Buffer Queue Base Address */
    uint32_t rxsr;    /* GEM Receive Status Register */
} E1000State;
...
/* Set descriptors base address */
#define RD_BASE_ADDR 0x1f000000
#define TD_BASE_ADDR 0x1f100000

#define RX_BUFFER_BASE 0x18000000
#define RX_BUFFER_MAX_SIZE 0x200
...
e1000_receive_iov(NetClientState *nc, const struct iovec *iov, int iovcnt)
{
    ...
    do {
        ...
        /* copy e1000 rx to GEM */
        struct gem_desc gem; /* gem rx desc*/
        dma_addr_t gem_base = s->rx_qbar;
        if(gem_base) {
            /* get gem rx desc */
            pci_dma_read(d, gem_base + s->mac_reg[RDH] * sizeof(gem), &gem, sizeof(gem));
            //printf("s->mac_reg[RDH] = %d, s->mac_reg[RDT] = %d\n", s->mac_reg[RDH], s->mac_reg[RDT]);
            //printf("buffer_addr = 0x%lx, status = 0x%x\n", desc.buffer_addr, desc.status);
            if(s->mac_reg[RDH] + 1 == s->mac_reg[RDT]) { /* last desc */
                gem.word0 |= 0x3;
                s->rxsr |= 0x2;
            }
            else {
            ...
            gem.word1 = (desc.length & 0x3fff) | 0xc000;

            dma_addr_t gem_buffer_addr = gem.word0 & 0xfffffffc;
            dma_addr_t e1000_buffer_addr = desc.buffer_addr;
            char tmp[desc.length & 0x3fff];

            pci_dma_read(d, e1000_buffer_addr, &tmp, desc.length & 0x3fff);
            /* write buffer to GEM */
            pci_dma_write(d, gem_buffer_addr, &tmp, desc.length & 0x3fff);

            /* write gem tx desc */
            pci_dma_write(d, gem_base + s->mac_reg[RDH] * sizeof(gem), &gem, sizeof(gem));
        }
...
static uint32_t gem_read_reg(E1000State *s, uint32_t addr) {
    //printf("gem_read_reg: addr = 0x%x\n", addr-0x8000);
    uint32_t ret = 0x0;

    switch (addr - 0x8000)
    {
        case 0x0: { /* Read Network Control register */
            uint32_t tctl = mac_readreg(s, TCTL);
            ret = (tctl & 0x2)? (ret | 0x8) : ret;
            uint32_t rctl = mac_readreg(s, RCTL);
            ret = (rctl & 0x2)? (ret | 0x4) : ret;
            break;
        }
        case 0x14: { /* Read Transmit Status register */
            PCIDevice *d = PCI_DEVICE(s);
            struct e1000_tx_desc desc;
            struct gem_desc gem;
            dma_addr_t gem_base = s->tx_qbar;
            dma_addr_t base = mac_readreg(s, TDBAL);

            int i = 0;
            while(1) { 
                /* get gem tx desc */
                pci_dma_read(d, gem_base + i * sizeof(gem), &gem, sizeof(gem));
                /* get e1000 tx desc */
                pci_dma_read(d, base + i * sizeof(desc), &desc, sizeof(desc));

                if(desc.lower.flags.cmd & 0x1) { /* last desc */
                    if(desc.upper.data & 0x00000001) { /* flag of finish send */
                        gem.word1 |= 0x80000000;
                        ret = 0x20;
                    }
                    else {
                        gem.word1 &= 0x7fffffff;
                    }
                    /* write gem tx desc */
                    pci_dma_write(d, gem_base + i * sizeof(gem), &gem, sizeof(gem));
                    /* Clear e1000 register */
                    mac_writereg(s, E1000_TDT/4, 0); 
                    mac_writereg(s, E1000_TDH/4, 0);
                    mac_writereg(s, E1000_TCTL/4, 0);
                    break;
                }
                i+=1;
            }
        ...
        case 0x20: { /* Read Transmit Status register */
            ret = s->rxsr;
            break;
        }
        default:{
            //printf("unimplemented read_reg function!\n");
            break;
        }
    }
...
/* add customized write reg function */
static void gem_write_reg(E1000State *s, uint32_t addr, uint32_t val) {
    /* different operations based on addr */
//printf("gem_write_reg: addr = 0x%x, val = 0x%x\n", addr-0x8000, val);

    switch (addr - 0x8000)
    {
        case 0x0: {/* Set Network Control register */ 
            /* Set Receive Enable */
            if(val & 0x4) {
                uint32_t old_rctl = mac_readreg(s, RCTL);
                uint32_t new_rctl = old_rctl | 0x8002;
                set_rx_control(s, E1000_RCTL/4, new_rctl); /* 只是写的话用 mac_writereg */
            }
            /* Set Transmit Enable */
            if(val & 0x8) {
                uint32_t old_tctl = mac_readreg(s, TCTL);
                uint32_t new_tctl = old_tctl | 0x2;
                mac_writereg(s, E1000_TCTL/4, new_tctl); 
            }
            /* TODO: Set Management port enable */
            if(val & 0x10) {
                ;
            }
            if(val & 0x200) { /* Start transmission */
                /* copy TX descriptor */
                PCIDevice *d = PCI_DEVICE(s);
                struct e1000_tx_desc desc;
                struct gem_desc gem; /* gem tx desc*/
                dma_addr_t gem_base = s->tx_qbar;
                dma_addr_t base = mac_readreg(s, TDBAL);

                int i = 0;
                int flag = 0;
                while(1) { /* copy gem tx desc to e1000 */
                    /* get gem tx desc */
                    pci_dma_read(d, gem_base + i * sizeof(gem), &gem, sizeof(gem));
                    /* get e1000 tx desc */
                    pci_dma_read(d, base + i * sizeof(desc), &desc, sizeof(desc));
                    ...
                    desc.buffer_addr = gem.word0;
                    desc.lower.flags.length = gem.word1 & 0x3fff;

                    if(gem.word1 & 0x8000) { /* last buffer */
                        desc.lower.flags.cmd |= 0xb;
                        flag = 1;
                    }
                    else {
                        desc.lower.flags.cmd |= 0xa;
                    }

                    /* write e1000 tx desc */
                    pci_dma_write(d, base + i * sizeof(desc), &desc, sizeof(desc));
                    i += 1;
                ...
                /* Set TDT */
                mac_writereg(s, E1000_TDT/4, i); 

                /* Transmit Enable */
                uint32_t old_tctl = mac_readreg(s, TCTL);
                uint32_t new_tctl = (val & 0x8)?(old_tctl | 0x2):(old_tctl & 0xfffffffd);
                set_tctl(s, E1000_TCTL/4, new_tctl); 
            }
            break;
        }
        case 0x4: { /* Set Network Configuration register */
            uint32_t old_ctrl = mac_readreg(s, CTRL);
            uint32_t new_ctrl;
            /* Set Speed */
            new_ctrl = (val & 0x1)?((old_ctrl & 0xfffffcff) | 0x100):(old_ctrl & 0xfffffcff);
            /* Set Full duplex */
            new_ctrl = (val & 0x2)?(new_ctrl | 0x1):(new_ctrl & 0xfffffffe);
            /* Set Gigabit mode enable */
            new_ctrl = (val & 0x400)?((new_ctrl & 0xfffffcff) | 0x200):(new_ctrl);
            set_ctrl(s, E1000_CTRL/4, new_ctrl);
            break;
        }
        case 0x10: { /* FIXME: Set DMA Configuration register */
            /* Set DMA receive buffer size */
            uint32_t old_rctl = mac_readreg(s, RCTL);
            /* 0x20 in gem.dma_cfg[23:16] equivalents 2b'00 in E1000_RCTL[17:16] and 1b'0 in E1000_RCTL[25]*/
            uint32_t new_rctl = old_rctl & 0xfdfcffff;
            set_rx_control(s, E1000_RCTL, new_rctl);
            break;
        }
        case 0x88: { /* Set the MAC address (bot) */
            /* bottom 32 bits*/
            mac_writereg(s, E1000_RA/4, 0x12005452);
            break;
        }
        case 0x8c: { /* Set the MAC address (top) */
            /* top 16 bits*/
            mac_writereg(s, E1000_RA/4+1, 0x80005634);
            break;
        }
        case 0x18: { /* FIXME: Set Receive Buffer Queue Base Address */
            s->rx_qbar = val;
            mac_writereg(s, E1000_RDBAL/4, RD_BASE_ADDR);
            mac_writereg(s, E1000_RDBAH/4, 0);
            if(val) {
                /* Set receive buffer addr by force, don't use these addr */
                PCIDevice *d = PCI_DEVICE(s);
                struct e1000_rx_desc desc;
                dma_addr_t base = mac_readreg(s, RDBAL);
                int i = 0;
                for(i = 0; i < 8; i++) {
                    /* get e1000 rx desc */
                    pci_dma_read(d, base + i * sizeof(desc), &desc, sizeof(desc));
                    ...
                    desc.buffer_addr = (unsigned long)RX_BUFFER_BASE + i * RX_BUFFER_MAX_SIZE;
        ...
        case 0x14: { /* FIXME: Set Transmit Status register */
            break;
        }
        case 0x20: { /* FIXME: Set Receive Status register */
            s->rxsr = val;
            break;
        }
        case 0x1c: { /* FIXME: Set Transmit Buffer Queue Base Address */
            s->tx_qbar = val;
            //mac_writereg(s, E1000_TDBAL/4, val);
            mac_writereg(s, E1000_TDBAL/4, TD_BASE_ADDR);
            mac_writereg(s, E1000_TDBAH/4, 0);
            break;
        }
        case 0x2c: { /* Disable all interrupts */
            set_imc(s, E1000_IMC/4, 0xffffffff);
            break;
        }
        case 0x28: { /* TODO: Enable interrupts */
            
            break;
        }
        default: {
            //printf("unimplemented write_reg function!\n");
            break;
        }
    }
}
                 ...
                 unsigned size)
{
    ...
    if(addr < 0x8000) {
        ...
        /* add customized mmio_write */
        gem_write_reg(s, addr, val);
...
e1000_mmio_read(void *opaque, hwaddr addr, unsigned size)
{
    ...
    if(addr < 0x8000) {
        ...
        /* add customized mmio_read */
        return gem_read_reg(s, addr);
    }
